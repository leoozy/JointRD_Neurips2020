...
class Distiller(nn.Module):
    ...
    def __init__(self, student, teacher, tblock_choices, sblock_choices, kd_type, num_classes):
        ...
        if sum([sum(i) for i in tblock_choices]) == 8:
            self.dis_point = sorted(set([1, 3, 5]))
        else:
    ...
    def forward(self, x, **kwargs):
        ...
        if stage == "TA" or stage == "JOINT":
            ...
            return self.forward_kd(x, **kwargs)
        elif "RES_NMT" in stage:
            return self.teacher(x, **kwargs)
        elif "RES_TA" in stage:
            return self.teacher.forward_to(x,  dis_point=[])
    ...
    def forward_kd(self, x, **kwargs):  
        fea_final_student = []
        out_final_student = []
        ##########run student####################

        student_logits, student_feas = self.student.forward_to(x, dis_point=[]) 
        ...
        if self.kd_type == "margin":
            ...
            for i, s_fea in enumerate(fea_final_student):
                ...
                loss_distill.append(distillation_loss(s_fea, teacher_feas.detach(), getattr(self, 'margin%d' % (1))) / (teacher_feas.detach().shape[0] + 1e-8))
